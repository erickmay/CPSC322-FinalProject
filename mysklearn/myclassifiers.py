...
class MyDecisionTreeWithF:
    """Represents a decision tree classifier WHERE F CAN BE SPECIFIED.
    Attributes:
        X_train(list of list of obj): The list of training instances (samples).
                The shape of X_train is (n_train_samples, n_features)
        y_train(list of obj): The target y values (parallel to X_train).
    ...
    Notes:
        ...
        Terminology: instance = sample = row and attribute = feature = column
    """
    def __init__(self, f, random_state):
        """Initializer for MyDecisionTreeClassifier.
        """
        self.X_train = None
        self.y_train = None
    ...
    def tdidt(self, current_instances, available_attributes):
        ...
        # select an attribute to split on
        candidate_atts = np.random.choice(available_attributes, self.f)
        split_attribute = self.select_attribute(current_instances, candidate_atts)
        # print("splitting on:", split_attribute)
        available_attributes.remove(split_attribute)
        # cannot split on this attribute again in this branch of tree
        tree = ["Attribute", split_attribute]
...
class MyRandomForestClassifier:
    ...
    def fit(self, X_train, y_train):
        ...
        for i in range(0, self.n):
            X_train_ind, X_val, y_train_ind, y_val = myevaluation.bootstrap_sample(self.X_train, self.y_train, len(self.X_train), i+1)
            train = [list(X_train_ind[i]) + [y_train_ind[i]] for i in range(len(self.X_train))]
            tree = self.tdidt(train, copy.deepcopy(self.header))
            y_pred = self.tree_predict(X_val, tree)
            accuracy = myevaluation.accuracy_score(y_val, y_pred)
            self.trees.append(tree)
            self.accuracies.append(accuracy)
        n = copy.deepcopy(self.n)
        while (self.m < n):
            index_to_remove = self.accuracies.index(min(self.accuracies))
            self.accuracies.remove(min(self.accuracies))
            self.trees.remove(self.trees[index_to_remove])
            n = n - 1
    ...
    def predict(self, X_test):
        ...
        for row in X_test:
            ...
            for tree in self.trees:
                y_pred = self.tree_predict(row, tree)
